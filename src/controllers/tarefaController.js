...
export const tarefaController = {
  ...
  async criar(req, res) {
    ...
    result.rowCount > 0 ? res.send("Tarefa cadastrada com sucesso!") : res.send("Erro ao cadastrar");
  },

  async atualizar(req, res) {
    const result = await tarefaService.atualizar(req.params.id, req.body.titulo, req.body.descricao, req.body.id_usuario);
    result.rowCount > 0 ? res.send("Tarefa atualizada com sucesso!") : res.send("Erro ao atualizar");
  },

  async deletar(req, res) {
    const result = await tarefaService.deletar(req.params.id);
    result.rowCount > 0 ? res.send("Tarefa excluída com sucesso!") : res.send("Erro ao excluir");
  }
};
