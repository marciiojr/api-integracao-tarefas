...
export const usuarioController = {
  ...
  async criar(req, res) {
    ...
    result.rowCount > 0 ? res.send("Usuário cadastrado com sucesso!") : res.send("Erro ao cadastrar");
  },

  async atualizar(req, res) {
    const result = await usuarioService.atualizar(req.params.id, req.body.nome, req.body.email, req.body.senha);
    result.rowCount > 0 ? res.send("Usuário atualizado com sucesso!") : res.send("Erro ao atualizar");
  },

  async deletar(req, res) {
    const result = await usuarioService.deletar(req.params.id);
    result.rowCount > 0 ? res.send("Usuário deletado com sucesso!") : res.send("Erro ao deletar");
  }
};
